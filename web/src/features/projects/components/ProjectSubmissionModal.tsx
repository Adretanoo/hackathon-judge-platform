...
import { ArrowLeft, Save, Code2, ExternalLink, GitBranch } from 'lucide-react';
...
export function ProjectSubmissionModal({ isOpen, onClose, teamId, projectId, initialData }: ProjectSubmissionModalProps) {
  ...
  const submitMutation = useMutation({
    mutationFn: (data: any) => 
      projectId 
        ? authClient.put(`/projects/${projectId}`, data)
        : authClient.post(`/projects`, { ...data, teamId }),
    onSuccess: () => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl p-0">
        ...
        <form onSubmit={handleSubmit((d) => submitMutation.mutate(d))} className="px-6 py-6 space-y-8 max-h-[70vh] overflow-y-auto">
          
          <div className="space-y-4">
            <h3 className="font-bold text-sm uppercase tracking-widest text-muted-foreground border-b pb-2">Основна інформація</h3>
            <div className="grid gap-2">
              <Label>Назва проєкту</Label>
              <Input {...register('name', { required: true })} placeholder="Назва вашого неймовірного продукту" className="font-bold text-lg" />
            </div>
            <div className="grid gap-2">
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-sm uppercase tracking-widest text-muted-foreground border-b pb-2">Посилання (Матеріали)</h3>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="grid gap-2">
                <Label className="flex items-center gap-2"><GitBranch className="h-4 w-4" /> Github Repository</Label>
                <Input {...register('githubUrl')} placeholder="https://github.com/..." type="url" />
              </div>
              <div className="grid gap-2">
              ...
              <div className="grid gap-2 md:col-span-2">
                <Label>Презентація / Pitch Deck (PDF або Slide URL)</Label>
                <Input {...register('presentationUrl')} placeholder="Посилання на слайди..." type="url" />
              </div>
            </div>
          </div>
